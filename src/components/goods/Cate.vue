  ...
  <div>
    ...
    <el-dialog title="修改分类" :visible.sync="editDialogVisible" width="50%" @close="editDialogClosed">
     <el-form :model="editForm" :rules="editFormRules" ref="editFormRef" label-width="100px">
        <el-form-item label="分类商品" prop="cat_name">
...
export default {
  data() {
    return {
      ...
      // 修改分类的验证规则
      editFormRules: {
        cat_name:[{ required: false, message: '请输入分类名称', trigger: 'blur' }]
       },
      // 父级分类的列表
      parentCateList: [],
      // 指定级联选择器的配置对象
  ...
  methods: {
   ...
   // 显示编辑对话框，注意看api接口查询分类的id以及获取分类的id
    ...
    editCateInfo(){
     this.$refs.editFormRef.validate(async valid => {
        ...
        if (res.meta.status !== 200) {
          return this.$message.error('更新信息失败了！')
        }
